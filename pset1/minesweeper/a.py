...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        # đánh dấu ô là một trong những nước đi được thực hiện trong trò chơi
        self.moves_made.add(cell)

        # mark the cell as a safe cell, updating any sequences that contain the cell as well
        # đánh dấu ô là ô an toàn, cập nhật bất kỳ trình tự nào chứa ô đó
        self.mark_safe(cell)

        # add new sentence to AI knowledge base based on value of cell and count
        # thêm câu mới vào cơ sở kiến ​​thức AI dựa trên giá trị của ô và số lượng
        cells = set()
        count_cpy = copy.deepcopy(count)
        close_cells = self.return_close_cells(cell)  # returns neighbour cells||trả về các ô hàng xóm
        for cl in close_cells:
            if cl in self.mines:
                count_cpy -= 1
            if cl not in self.mines | self.safes:
                cells.add(cl)  # only add cells that are of unknown state||chỉ thêm các ô có trạng thái không xác định

        new_sentence = Sentence(cells, count_cpy)  # prepare new sentence||chuẩn bị câu mới

        if len(new_sentence.cells) > 0:  # add that sentence to knowledge only if it is not empty||chỉ thêm câu đó vào kiến ​​thức nếu nó không trống
        ...
        # check sentences for new cells that could be marked as safe or as mine||kiểm tra các câu cho các ô mới có thể được đánh dấu là an toàn hoặc là của tôi
